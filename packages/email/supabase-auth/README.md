...
- `confirmation.html`: Used for the "Confirm Signup" flow when new users register.
...
- `recovery.html`: Used for the "Reset Password" flow.
- `reauthentication.html`: Used when re-verifying a user's session for sensitive actions.
- `change_email.html`: Used when a user updates their email address.
